...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_lodash_function_debounce__ = __webpack_require__(260);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_lodash_function_bind__ = __webpack_require__(438);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__page_manager__ = __webpack_require__(246);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_jquery__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_jquery___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3_jquery__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__common_gift_certificate_validator__ = __webpack_require__(419);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__bigcommerce_stencil_utils__ = __webpack_require__(45);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__cart_shipping_estimator__ = __webpack_require__(431);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__global_modal__ = __webpack_require__(99);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_sweetalert2__ = __webpack_require__(71);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__custom_cart_personal_notes__ = __webpack_require__(432);
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_lodash_collection_each__ = __webpack_require__(402);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_lodash_lang_isEmpty__ = __webpack_require__(255);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_lodash_object_transform__ = __webpack_require__(411);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__form_utils__ = __webpack_require__(381);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_sweetalert2__ = __webpack_require__(71);
...
var arrayEach = __webpack_require__(251),
    baseEach = __webpack_require__(254),
    createForEach = __webpack_require__(410);
...
var arrayEach = __webpack_require__(251),
    baseCallback = __webpack_require__(253),
    baseCreate = __webpack_require__(394),
    baseForOwn = __webpack_require__(102),
    isArray = __webpack_require__(16),
    isFunction = __webpack_require__(256),
...
var baseCreate = __webpack_require__(394),
    baseLodash = __webpack_require__(421);
...
var identity = __webpack_require__(105),
    metaMap = __webpack_require__(429);
...
var metaMap = __webpack_require__(429),
    noop = __webpack_require__(449);
...
/* WEBPACK VAR INJECTION */(function(global) {var getNative = __webpack_require__(70);
...
var baseSetData = __webpack_require__(425),
    now = __webpack_require__(259);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__common_state_country__ = __webpack_require__(399);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__common_nod__ = __webpack_require__(375);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__bigcommerce_stencil_utils__ = __webpack_require__(45);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__common_form_utils__ = __webpack_require__(381);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_sweetalert2__ = __webpack_require__(71);
...
var PERSONAL_NOTE='Personal Note';var PERSONAL_NOTE_WRAPPING_ID=null;// Used to know what's the Personal Note Gift wrapping option id
/**
 * We just the element the invisible class if present
 * @param {DOMElement}  $element
 */function hide($element){if($element)$element.classList.add('cart-notes--invisible')}/**
 * We remove the invisible class on the element if present
 *
 * @param {DOMElemetn} $element
 */function unhide($element){if($element)$element.classList.remove('cart-notes--invisible')}/**
 ...
 */function sendNote(itemId,note,cartInstance){if(PERSONAL_NOTE_WRAPPING_ID===null){throw new Error('No "Personal Note" Gift Wrapping option available to use')}var request=new XMLHttpRequest;/* eslint no-console: off */request.open('POST','/cart.php');request.setRequestHeader('Content-Type','application/x-www-form-urlencoded');request.addEventListener('error',function(error){return console.error(error)});request.addEventListener('load',function(){return cartInstance.refreshContent.bind(cartInstance)()});var formData='';var urlEncodedDataPairs=[];var data={action:'save_giftwrapping','giftmessage[all]':note,'giftwrapping[all]':PERSONAL_NOTE_WRAPPING_ID,giftwraptype:'same',item_id:itemId};/* eslint guard-for-in: off */for(var name in data){urlEncodedDataPairs.push(encodeURIComponent(name)+'='+encodeURIComponent(data[name]))}// we save the message on the local storage in case BigCommerce truncates our message later
localStorage.setItem('note-'+itemId,note);formData=urlEncodedDataPairs.join('&').replace(/%20/g,'+');request.send(formData)}/**
 * We check that no personal notes widget is shown for Custom bon boxes
 */function checkCustomProducts(){var customRegexp=/CUSTOM BON BOX/;Array.from(document.querySelectorAll('.cart-notes-container')).forEach(function($personalNoteContainer){if(customRegexp.test($personalNoteContainer.getAttribute('data-product-name'))===false){var $addButton=$personalNoteContainer.querySelector('.cart-notes--add-button');if($addButton)$addButton.classList.remove('cart-notes--invisible')}else{var $editButton=$personalNoteContainer.querySelector('.cart-notes--edit-button');if($editButton)$personalNoteContainer.classList.add('cart-notes--invisible')}})}/**
 ...
 * @param {string}   itemId
 * @param {function} callback
 */function getWrappingOptions(itemId){var giftwrapUrl='/remote/v1/gift_wrapping/'+itemId;var request=new XMLHttpRequest;request.open('GET',giftwrapUrl);request.setRequestHeader('accept','application/json');request.addEventListener('load',function(event){if(!event||'responseText'in event.target===false){return}var data=null;try{data=JSON.parse(event.target.responseText)}catch(error){console.error('Malformed JSON when getting wrapping options');console.error(error)}if(!data){return}var giftWrappingFiltered=data.data.gift_wrappings.filter(function(gw){return gw.name===PERSONAL_NOTE});if(giftWrappingFiltered.length===0){return}PERSONAL_NOTE_WRAPPING_ID=giftWrappingFiltered[0].id});request.send()}/**
 ...
 */function checkAllMessages(){var ellipsisRegexp=/\.\.\.$/;Array.from(document.querySelectorAll('p.cart-notes-message')).forEach(function($note){if(ellipsisRegexp.test($note.textContent)===false)return;var itemId=$note.getAttribute('data-item-note');var localNote=localStorage.getItem('note-'+itemId);if(localNote)$note.textContent=localNote})}function init(cartInstance){checkCustomProducts();checkAllMessages();var $personalNotes=Array.from(document.querySelectorAll('.cart-notes-container'));var isGetWrappingOptionsCalled=false;$personalNotes.forEach(function($personalNoteContainer){// We try to get the wrapping options if present since Custom boxes don't have it
if(isGetWrappingOptionsCalled===false){var $dataItemNote=$personalNoteContainer.querySelector('[data-item-note]');if($dataItemNote){getWrappingOptions($dataItemNote.getAttribute('data-item-note'));isGetWrappingOptionsCalled=true}}$personalNoteContainer.addEventListener('click',function(event){var target=event.target;// When we click on Add button, we need to hide this button and show the text area
if(target.classList.contains('cart-notes--add-button')){hide(target);unhide($personalNoteContainer.querySelector('.cart-notes-input-container'))}// When clicking on Cancel on text area, we need to show the Add button if we're adding
// or Edit Button if we're editing
if(target.classList.contains('cart-notes-input-container--cancel-button')){hide($personalNoteContainer.querySelector('.cart-notes-input-container'));unhide($personalNoteContainer.querySelector('.cart-notes--add-button'));unhide($personalNoteContainer.querySelector('.cart-notes-message-container'))}// When clicking the Edit button we need to copy what's on the message to the textarea, so even if the customer cancels the edition
// and goes back to it, always has the message
if(target.classList.contains('cart-notes--edit-button')){/* eslint no-param-reassign: off  */$personalNoteContainer.querySelector('textarea').value=$personalNoteContainer.querySelector('.cart-notes-message').textContent;hide($personalNoteContainer.querySelector('.cart-notes-message-container'));unhide($personalNoteContainer.querySelector('.cart-notes-input-container'))}// When clicking Save button, we need to send the information and reload?
if(target.classList.contains('cart-notes-input-container--save-button')){var $textarea=$personalNoteContainer.querySelector('textarea');if(!$textarea)throw new Error('There is no text area when adding a personal note');hide(target);var textAreaInfo=$textarea.value.trim();sendNote(target.getAttribute('data-item-note'),textAreaInfo,cartInstance)}})})}
...
var LazyWrapper = __webpack_require__(420),
    LodashWrapper = __webpack_require__(424),
    baseLodash = __webpack_require__(421),
    isArray = __webpack_require__(16),
    isObjectLike = __webpack_require__(34),
    wrapperClone = __webpack_require__(448);
...
var createWrapper = __webpack_require__(442),
    replaceHolders = __webpack_require__(423),
...
/* WEBPACK VAR INJECTION */(function(global) {var createCtorWrapper = __webpack_require__(422);
...
/* WEBPACK VAR INJECTION */(function(global) {var arrayCopy = __webpack_require__(252),
    composeArgs = __webpack_require__(426),
    composeArgsRight = __webpack_require__(427),
    createCtorWrapper = __webpack_require__(422),
    isLaziable = __webpack_require__(444),
    reorder = __webpack_require__(447),
    replaceHolders = __webpack_require__(423),
    setData = __webpack_require__(430);
...
var baseSetData = __webpack_require__(425),
    createBindWrapper = __webpack_require__(439),
    createHybridWrapper = __webpack_require__(440),
    createPartialWrapper = __webpack_require__(441),
    getData = __webpack_require__(428),
    mergeData = __webpack_require__(445),
    setData = __webpack_require__(430);
...
var realNames = __webpack_require__(446);
...
var LazyWrapper = __webpack_require__(420),
    LodashWrapper = __webpack_require__(424),
    arrayCopy = __webpack_require__(252);
